...
### MAKE THIS STACKED with one combined error bar
seasonal_flux_plot <- ggplot(seasonal_fluxes, aes(x = factor(season, levels=season_order), y = combined_mean_flux, fill = niche,ymin = total_flux - se, ymax = total_flux + se)) + 
...
## or
ggplot(seasonal_fluxes, aes(x = niche, y = combined_mean_flux, fill = niche)) + geom_errorbar(aes(ymin = combined_mean_flux - 0, ymax = combined_mean_flux + se), width = 0.25, position = dodge) +
  geom_col(position = dodge) + facet_wrap(~factor(season, levels = season_order), scales = "free_y") +
  labs(title = "N fluxes from cryptic fixation across seasons") +
  ylab(label = "Cryptic N fixation flux (kg N ha-1 season-1)") + 
  xlab(label = "Season") +
  theme(text = element_text(size=20))
  
### now embed summer dry vs wet plot in the seasonal flux plot
rain_fluxes <- cryptic_rates %>%
...
rain_flux_sum <- rain_fluxes %>% group_by(season, niche) %>%
  summarize(mean_AR_rate = mean(flux_nmol_C2H4_g_h),
            mean_daily_N_fix = mean(flux_gramN_g_h)*24,
            sample_n = length(flux_nmol_C2H4_g_h),
            SE_AR_rate = sd(flux_nmol_C2H4_g_h)/sqrt(sample_n),
            mean_moist = mean(moist))

rain_plot<- ggplot(rain_flux_sum, aes(x = factor(season, levels = c("before", "after")), y =mean_AR_rate, fill = niche)) + 
         geom_col(position = dodge) + facet_wrap(~niche, scales = "free") +
  geom_errorbar(aes(ymin = mean_AR_rate - SE_AR_rate, ymax = mean_AR_rate + SE_AR_rate), width = 0.25, position = dodge)+
         labs(title = "Cryptic AR rates before and after rain") +
         ylab(label = "Cryptic AR rate (nmol C2H4 g-1 hr-1)") + 
...
ggplot(rain_flux_sum, aes(x = factor(season, levels = c("before", "after")), y =mean_daily_N_fix, fill = niche)) + 
  geom_boxplot() + facet_wrap(~niche, scales = "free") +
  labs(title = "Cryptic AR rates before and after rain") +
  ylab(label = "Cryptic AR rate (nmol C2H4 g-1 hr-1)") + 
  xlab(label = element_blank()) +
  theme(text = element_text(size=18), legend.position = "none", plot.background = element_rect(colour = 'black', fill = 'white', linetype='solid')) 

### comparison of moist (t-test) between before and after rain
rain_fluxes %>% group_by(niche) %>%
  group_modify(~broom::tidy(t.test(moist~season, .x)))


seasonal_flux_plot + annotation_custom(ggplotGrob(rain_plot), xmin = 1.35, xmax = 3.5, 
                                       ymin = 0.1, ymax = 0.29)
##### N fix x Moist x season plot #####
cryptic_rates_sum <- cryptic_rates %>% group_by(season, niche, site) %>%
  summarize(mean_AR_rate = mean(flux_nmol_C2H4_g_h),
            sample_n = length(flux_nmol_C2H4_g_h),
            SE_AR_rate = sd(flux_nmol_C2H4_g_h)/sqrt(sample_n),
            mean_moist = mean(moist), 
            SE_moist = sd(moist)/sqrt(sample_n))

### change summer and summer_wet to "before rain" and "after rain"
cryptic_rates_sum$season[cryptic_rates_sum$season == "summer"] <- "summer (before rain)"
cryptic_rates_sum$season[cryptic_rates_sum$season == "summer_wet"] <- "summer (after rain)"
## remove soil moisture outlier
cryptic_rates_red <- cryptic_rates_sum %>% filter(mean_moist <300)

ggplot(cryptic_rates_red, aes(x = mean_moist, y =mean_AR_rate, color = niche)) + 
  geom_point(size = 3)  + facet_wrap(~niche, scales = "free") +
  geom_errorbar(aes(ymin = mean_AR_rate - SE_AR_rate, ymax = mean_AR_rate + SE_AR_rate), width = 10)+
  labs(title = "Cryptic AR rates in response to field moisture") +
  ylab(label = "Cryptic AR rate (nmol C2H4 g-1 hr-1)") + 
  xlab(label = "Mean moisture content (%w/w)") +
  theme(legend.position = c(.8,.25), legend.text = element_text(size = 20),legend.title = element_text(size = 20)) +
  theme(legend.background = element_rect(colour = 'black', fill = 'white', linetype='solid')) +
...
#### Nfix by biomass or coverage ####
## Moss and lichens
## first summarize by plot across all seasons
ML_flux_cover_sum <- moss_lichen_fluxes %>% group_by(niche, site) %>%
  summarise(mean_flux = mean(sum(flux_kgN_ha_season)),
            SE_flux = sqrt(sum(sd_kgN_ha_season^2))/sqrt(sum(sample_n)),
            mean_cover = mean(mean.cover.percent))
ggplot(ML_flux_cover_sum, aes(x = mean_cover, y = mean_flux, color = niche)) + 
  geom_point(size=5) + geom_smooth(method='lm', se =FALSE, linetype = "dashed") +
  geom_errorbar(aes(ymin = mean_flux-SE_flux, ymax =mean_flux+SE_flux, width = 0.5)) +
  labs(title = "N inputs from autotrophic N fixation in relation to fixer coverage") +
  ylab(label = "N fixation flux (kg N ha-1 season-1)") + 
  xlab(label = "Plot level coverage (%)") +
  theme(strip.text.x = element_text(size = 20)) +
  theme(axis.text = element_text(size=20)) +
  theme(title = element_text(size=20)) +
  theme(legend.position = c(.875,.245),
        legend.title = element_blank(), 
        legend.background = element_rect(colour = 'black', fill = 'white', linetype='solid'),
        legend.text = element_text(size = 20))
  
## Wood N fix x Wood biomass ##
wood_flux_bm_sum <- wood_fluxes %>%
  group_by(site) %>%
  summarise(mean_flux = mean(sum(flux_kgN_ha_season)),
            SE_flux = sqrt(sum(sd_kgN_ha_season^2))/sqrt(sum(sample_n)),
            CWD_Mg_ha = mean(bm_1000h_Class45_Mg_ha))
ggplot(wood_flux_bm_sum, aes(x = CWD_Mg_ha, y = mean_flux)) + 
         geom_point(size=5) + geom_smooth(method='lm', se =FALSE, linetype = "dashed") +
         geom_errorbar(aes(ymin = mean_flux-SE_flux, ymax =mean_flux+SE_flux, width = 0.5)) +
         labs(title = "Heterotrophic N fixation in deadwood") +
         ylab(label = "N fixation flux (kg N ha-1 season-1)") + 
         xlab(label = "Plot level CWD (Mg biomass ha-1))") +
         theme(strip.text.x = element_text(size = 20)) +
         theme(axis.text = element_text(size=20)) +
         theme(title = element_text(size=20)) +
         theme(legend.position = c(.875,.245),
...
# soil rate comparison
cryptic_rates %>% filter(niche == "Soil") %>% group_by(season) %>% summarize(mean_rate_ug_N = mean(flux_gramN_g_h)*1000000,
                                                                             mean_rate_nmol_C2H4 = mean(flux_nmol_C2H4_g_h),
                                                                             sd_nmolC2H4 = sd(flux_nmol_C2H4_g_h))

cryptic_rates %>% filter(niche == "Soil") %>% summarize(mean_rate_ug_N = mean(flux_gramN_g_h)*1000000,
                                                                             mean_rate_nmol_C2H4 = mean(flux_nmol_C2H4_g_h),
                                                                             sd_nmolC2H4 = sd(flux_nmol_C2H4_g_h))


summary(glm(flux_nmol_C2H4_g_h ~ season+niche+site+moist, data = cryptic_rates))
summary(lm(flux_nmol_C2H4_g_h ~ season+niche+site+moist, data = cryptic_rates))

anova(lm(flux_nmol_C2H4_g_h ~ season+niche+site+moist, data = cryptic_rates))
